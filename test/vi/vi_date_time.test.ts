...
test("Test - HHmmDDMMYY", function () {
    testSingleCase(chrono.vi, "bắt đầu từ 5 giờ 0 phút đến 9 giờ 0 phút lúc ngày 25 tháng 3 năm 2021", new Date(2016, 10 - 1, 1, 11), (result, text) => {
        expect(result.start).toBeDate(new Date(2021, 3 - 1, 25, 5, 0, 0));
        expect(result.end).toBeDate(new Date(2021, 3 - 1, 25, 9, 0, 0));
    });
});
...
//     });
// });
test("Test - HHMM 8 giờ đến 9 giờ", function () {
